...
from pulp_docker.extensions.admin import copy_images


class TestDockerImageCopyCommand(unittest.TestCase):

    @patch('pulp_docker.extensions.admin.copy_images.get_formatter_for_type')
    def test_get_formatter_for_type(self, mock_formatter):
        context = MagicMock()
        command = copy_images.ImageCopyCommand(context)
...
class TestGetFormatterForType(unittest.TestCase):
    ...
    def test_get_formatter_for_type(self):
        formatter = copy_images.get_formatter_for_type(constants.IMAGE_TYPE_ID)
        self.assertEquals('foo', formatter({'image_id': 'foo'}))

    def test_get_formatter_for_type_raises_value_error(self):
        self.assertRaises(ValueError, copy_images.get_formatter_for_type, 'foo-type')
