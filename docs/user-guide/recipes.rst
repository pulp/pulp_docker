...
publish is placing metadata files. By default this is the ``/var/lib/pulp/published/docker/app/``
directory. Unless specified otherwise in /etc/crane.conf, Crane will check the ``data_dir`` for
updates once every 60 seconds. Full documentation for /etc/crane.conf can be found in the
`Crane`_ readme.
